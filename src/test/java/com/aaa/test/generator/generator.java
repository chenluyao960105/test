...
public class generator {
    public static void main(String[] args) {
        ...
        sourceConfig.setPassword("960105");
        sourceConfig.setUrl("jdbc:mysql://localhost:3306/test?characterEncoding=utf8");
        mpg.setDataSource(sourceConfig);
        // 策略配置
        StrategyConfig strategyConfig = new StrategyConfig();
        // .setCapitalMode(true)// 全局大写命名
        strategyConfig.setTablePrefix(new String[]{"tab_"});// 此处可以修改为您的表前缀
        strategyConfig.setNaming(NamingStrategy.underline_to_camel);// 表名生成策略
        mpg.setStrategy(strategyConfig);
        // 包配置
        PackageConfig packageConfig = new PackageConfig();
        packageConfig.setParent("com.aaa.test");// 自定义包路径
